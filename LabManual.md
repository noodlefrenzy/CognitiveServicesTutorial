...
Once you click it, you'll be presenting with the fields above to fill out. Choose your storage account name (lowercase letters and numbers), set _Account kind_ to _Blob storage_, _Replication_ to _Locally-Redundant storage (LRS)_ (this is just to save money), use the same Resource Group as we used for our previous steps. _Pin to dashboard_ so that you can easily find it.
